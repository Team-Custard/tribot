module.exports={
  ...
  fulldescription:"Adds the mute role to the user",
  ...
  load(client){
    client.registerCommand(module.exports.name,async(message,commandName,args)=>{
      ...
      if (!message.mentions.members.size){
        ...
        embed:{
          description:":no_entry_sign: No user was specified to mutr.",
          color:"RED"
        }
        }).catch(()=>{})
        }
        ...
        member.roles.add(role,`[mute by ${message.author.tag}]: ${r}`)
        .then(()=>{message.channel.send({
          embed:{
            description:`✅ Added **\`Muted\`** to ${member.user.tag}`,
            color:"GREEN"
          }
        }).catch(()=>{})
        })
